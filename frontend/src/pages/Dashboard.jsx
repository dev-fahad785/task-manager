...
function Dashboard() {
  ...
  // Sort tasks by due date - made it a pure function
  const sortTasksByDueDate = (tasksToSort) => {
    ...
    return [...tasksToSort].sort((a, b) => {
      const dueDateA = new Date(a.dueDate);
      const dueDateB = new Date(b.dueDate);
      const minutesRemainingA = (dueDateA - now) / (1000 * 60);
      const minutesRemainingB = (dueDateB - now) / (1000 * 60);
      return minutesRemainingA - minutesRemainingB;
    });
  };
  ...
  useEffect(() => {
    if (tasks2.length > 0) {
      filterTaskByCompletionStatus(activeFilter);
    } else {
      setFilteredTasks([]);
    }
  }, [tasks2, activeFilter]); // Removed filterTaskByCompletionStatus from dependencies
              ...
              <div className="space-y-4">
                {filteredTask.length === 0 ? (
                  ...
                  filteredTask.map((task) => (
                    ...
                    shadow hover:shadow-lg transition-all p-4 ${
                      task.completionStatus === "Completed" ? "opacity-60" : ""
                    }`}
                    >
                      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                        <div className="flex-1">
